      ...
      <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-3">
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-800 sm:w-80"
          />
          ...
          >
            ...
            <option value="tutorial">帖子</option>
          </select>
        </div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <button
            ...
            className="h-8 rounded-lg border border-rose-300 bg-white px-3 text-xs text-rose-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            批量删除
          </button>
          <button
            ...
            disabled={selectedCount === 0 || updating}
            className="h-8 rounded-lg border border-emerald-300 bg-white px-3 text-xs text-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            批量上架
          </button>
          <button
            ...
            disabled={selectedCount === 0 || updating}
            className="h-8 rounded-lg border border-amber-300 bg-white px-3 text-xs text-amber-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            批量下架
          </button>
          <span className="ml-auto text-xs text-slate-500">已选 {selectedCount} 项</span>
        </div>
      </section>
            ...
            <li key={key} className="flex h-full flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-auto pt-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link href={editPath(item.type, item.id)} className="rounded-lg border border-slate-300 px-2 py-1 text-xs text-slate-700 hover:bg-slate-50">
                    继续编辑
                  </Link>
                </div>
              </div>
